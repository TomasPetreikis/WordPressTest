...
function twentig_twentyone_body_class( $classes ) {
	...
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		...
		if ( $widget_background || $footer_background ) {
			$classes[] = 'tw-footer-widgets-bg';
			if ( $footer_background === $widget_background || ( $footer_background && empty( $widget_background ) ) ) {
...
function twentig_twentyone_print_customizer_css() {
	...
	if ( $post_h1_font_size ) {
		$css .= '@media( min-width:822px ) { .single-post { --global--font-size-page-title:' . twentig_twentyone_generate_value( $post_h1_font_size, 'rem' ) . '} }';
	}
...
function twentig_twentyone_has_sidebar() {
	if (
		...
		( get_theme_mod( 'twentig_blog_sidebar', false ) && ( is_home() || is_archive() || ( is_search() && 'blog-inherit' === get_theme_mod( 'twentig_page_search_layout' ) ) ) )
		|| ( get_theme_mod( 'twentig_post_sidebar', false ) && is_single() )
		)
	) {
		return true;
	}
